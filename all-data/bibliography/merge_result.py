...
argparser= argparse.ArgumentParser('merge the bibliography result from all folders')
argparser.add_argument('infile')
args=argparser.parse_args()

selected_files = args.infile

folders = []
# selected_files = folder_list_x.txt
with open('splitted_folders_files/' + selected_files, "r") as f:
    for line in f:
        folders.append(line.strip())

dfs = [ddf.read_parquet(f) for f in folders]
...
selected_files_index = ''.join(c for c in selected_files if c.isdigit())
file_name = 'matched_ids_' + selected_files_index + '.json'
import json
path1 = 'matched_ids/'
json.dump(matched_ids, open(path1 + file_name, 'w'))

x_df = ddf.from_pandas(new_ref_list,chunksize=10000)
path2 = 'ref_info/'
file_name2 = 'ref_info_' + selected_files_index
x_df.to_parquet(path2 + file_name2)
